...
try {
    ...
    $statement = $databaseConnection->prepare("INSERT INTO mblxdev.tblcommodities (name, description, uom) VALUES (?, ?, ?)");
    $statement->bind_param('sss', $name, $description, $uom);
    
   
    $name  = test_input($dataPosted["name"]);
    $description = test_input($dataPosted["desc"]);
